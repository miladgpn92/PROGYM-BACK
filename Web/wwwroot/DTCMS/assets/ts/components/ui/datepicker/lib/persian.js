...
(function (root, factory) {
  ...
  DateRangePicker.prototype = {
    ...
    monthOrYearChanged: function (e) {
      ...
      var year = this.persianObject.persianToEnglishDigits(
        cal.find(".yearselect").val()
      );
      ...
      if (!isLeft) {
        ...
        if (this.linkedCalendars)
          if (
            this.rightCalendar.clone().toDate() >
            this.leftCalendar.clone().toDate()
          ) {
    ...
    persianObject: {
      ...
      monthOrYearChanged: function (e) {
        ...
        // Month must be Number for new moment versions
        var jYear = this.persianObject.persianToEnglishDigits(
          cal.find(".yearselect").val()
        );
        ...
        if (!isLeft) {
          ...
          if (this.linkedCalendars)
            if (
              this.rightCalendar.month.clone().toDate() <
              this.leftCalendar.month.clone().toDate()
            ) {
              this.leftCalendar.month = this.rightCalendar.month
                .clone()
                .add(1, "month");
      ...
      renderCalendar: function (side) {
        ...
        var jFirstDay = moment
          .from([jYear, jMonth, 1].join("/"), "fa", "YYYY/MM/DD")
          .locale("en");
        var jDaysInMonth = jFirstDay.clone().locale("fa").daysInMonth();
        var jLastDay = moment.from(
          [jYear, jMonth, jDaysInMonth].join("/"),
          "fa",
          "YYYY/MM/DD"
        );
          ...
          if (i + 1 === jDaysInMonth && cEnddayOfWeekOffset) {
            while (cEnddayOfWeekOffset != 0) {
              ...
              if (calendar[row].length > 6) {
                calendar[++row].push(
                  curDate.clone().hour(hour).minute(minute).second(second)
                );
              } else {
                calendar[row].push(
                  curDate.clone().hour(hour).minute(minute).second(second)
        ...
        var dateHtml =
          this.locale.monthNames[calendar[1][1].format("jM") - 1] +
          this.persianObject.englishToPersianDigits(
            calendar[1][1].format(" jYYYY")
          );

        if (this.showDropdowns) {
          var jCurrentMonth = parseInt(
            moment(calendar[0][calendarFirstDay]).format("jM")
          );
          var currentMonth = parseInt(
            moment(calendar[0][calendarFirstDay]).format("M")
          );
          var m = 1;
          if (side === "right") {
          ...
          var currentYear = calendar[0][calendarFirstDay]
            .toDate()
            .getFullYear();
          var jCurrentYear = moment(calendar[0][calendarFirstDay]).format(
            "jYYYY"
          );
          var maxYear = (maxDate && maxDate.year()) || this.maxYear;
          var minYear = (minDate && minDate.year()) || this.minYear;
          ...
          for (m; m <= 12; m++) {
            ...
            if (
              (!inMinYear || (minDate && m > mcm)) &&
              (!inMaxYear || (maxDate && m < mcx))
            ) {
              monthHtml +=
                "<option value='" +
                m +
                "'" +
                (m == jCurrentMonth ? " selected='selected'" : "") +
                ">" +
                this.locale.monthNames[m - 1] +
                "</option>";
            } else {
              if (side === "right") {
                monthHtml +=
          ...
          for (var y = minYear; y <= maxYear; y++) {
            ...
            yearHtml +=
              ...
              jY +
              '"' +
              (jY === jCurrentYear ? ' selected="selected"' : "") +
              ">" +
              this.persianObject.englishToPersianDigits(jY) +
              "</option>";
            date.add(1, "year");
          }
          yearHtml += "</select>";
        ...
        if (
          ...
          html +=
            '<th class="prev available"><span class="persian-daterangepicker-prev"></span ></th>';
        } else {
          html += "<th></th>";
        }
        ...
        if (
          (!maxDate || maxDate.isAfter(calendar.lastDay)) &&
          (!this.linkedCalendars || side == "left" || this.singleDatePicker)
        ) {
          html +=
            '<th class="next  available"><span class="persian-daterangepicker-next"></span ></th>';
        } else {
          html += "<th></th>";
        }
        ...
        for (var row = 0; row < 7; row++) {
          ...
          for (var col = 0; col < 7; col++) {
            if (
              typeof calendar[row] !== "undefined" &&
              typeof calendar[row][col] !== "undefined"
            ) {
              var classes = [];
              ...
              // highlight weekends
              if (calendar[row][col].isoWeekday() > 6) classes.push("weekend");
              ...
              //don't allow selection of dates before the minimum date
              if (
                typeof offsetAddress[0] !== "undefined" &&
                offsetAddress[0] === col
              ) {
                classes.push("off", "disabled");
