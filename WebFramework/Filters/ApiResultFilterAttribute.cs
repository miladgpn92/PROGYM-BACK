...
namespace WebFramework.Filters
{
    public class ApiResultFilterAttribute : ActionFilterAttribute
    {
        public override void OnResultExecuting(ResultExecutingContext context)
        {
            ...
            else if (context.Result is ObjectResult objectResultItem)
            {
                if (objectResultItem.StatusCode == 200)
                {
                    var apiResult = new ApiResult<object>(true, ApiResultStatusCode.Success, objectResultItem.Value);
                    context.Result = new JsonResult(apiResult) { StatusCode = objectResultItem.StatusCode };
                }
